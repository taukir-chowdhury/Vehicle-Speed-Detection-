...
output_name = '2 01 R 14022022090000Am_output'
...
def adding_to_report(frame,ID,speed,time, xmin, ymin, xmax, ymax):
    ...
    if int(time/60) == 60:
        time = time % 60
    clock_time = f"{int(TIME_START)+1}:{int(time/60)}:{int(time%60)} {am_pm}"
    cv2.imwrite(f"{os.path.join(result_path,input_name,'saved_images')}/{name}.png", cropped_image)
    data.append([name,clock_time,speed])

    
def create_report(data,i):
    header = ['name', 'time(s)', 'speed(km/h)']
    with open(f"{result_path}/{input_name}/report({i}).csv", 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
