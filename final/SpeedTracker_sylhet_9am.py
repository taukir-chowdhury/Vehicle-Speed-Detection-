...
from yolov5_testing import get_masked_image
...
am_pm = 'am'
TIME_START = '9'
input_name = "2 03 R 14022022090000Am_test"
...
area = [(813,426), (1306,486),(1240, 541), (772,481), (210,650), (852, 789), (798, 832), (144,681)]
dr1 = [(0,650), (340,1000)]
distance_line = [(1842,477), (1905,477), (1900, 1055), (1780,1057)]
...
def get_tracking_id(result,frame, threshold = 0.5):
    
    test = result.xyxy[0]
    temp = test[test[:, 5] > 0]
    test2 = temp[temp[:,5] < 8] 
    test3 = test2[test2[:,4]>threshold]
    xywhs = xyxy2xywh(test3[:, 0:4])
    confs = test3[:, 4]
    clss =  test3[:, 5]
...
#                          FPS, size)
def get_masked_image(frame, roi):
    ...
    if success:
        ...
        if start_from_checkpoint:
            if FRAME_COUNT > checkpoint:
                start_from_checkpoint = False
                print("\nLoaded from Checkpoint: Checkpoint:",checkpoint,"Frame_count:", FRAME_COUNT)
        ...
        if if_skip_frame():
            ...
            FRAME_COUNT += 1
            if FRAME_COUNT % 1440==0:
                create_report(data,FRAME_COUNT)
            continue

        #print(FRAME_COUNT)

        frame = get_masked_image(frame)
        result = model(frame)
